...
#define MAX_RANGE       0x7FFF
		...
		LPARAM lParam)
	{
		...
		case WM_INITDIALOG: {
			...
			if (!GetSystemDirectory(psWinSysDir, MAX_PATH)) {
				...
				MessageBox(0,
					TEXT("Couldn't determine the system directory"),
					TEXT("Launch Driver Error"),
					MB_ICONERROR);
			...
			case IDC_LOAD_FILE: {
				...
				if (CheckFileEnding(psDriverFile, "sys")) {
					...
					if (image) {
						...
						MessageBox(0,
							TEXT("Couldn't determine the file name!!!"),
							TEXT("Launch Driver Error"),
							MB_ICONERROR);
						break;
					}
				...
				case INSTALLED:
					...
					if (check_access) {
						...
						if (!CheckAccess())
							...
							MessageBox(0,
								TEXT("Couldn't access the driver!!!"),
								TEXT("Launch Driver Error"),
								MB_ICONERROR);
					}
			...
			case ID_INSTALL:
				...
				if (inf_file_present == TRUE) {
					...
					if (!InstallDriverViaSetupApi()) {
						...
						wsprintf(errormsg,
							TEXT("Couldn't install kernel driver via INF file!\nErrorcode: 0x%x"),
							GetLastError());
	...
	*/
	...
	error:
		...
		wsprintf(errormsg,
			TEXT("Couldn't create the registry keys!!!\nErrorcode: %d"),
			errorcode);
	...
	*/
	BOOL WINAPI CopyDriver() {
		...
		if (!CopyFile(driver_file.psDriverFile,
			...
			wsprintf(errormsg,
				TEXT("Couldn't copy driver file to destination %s \nErrorcode: %d"),
				dstpath,
				errorcode);
	...
	*/
	...
	SC_HANDLE WINAPI OpenServiceDatabase(VOID) {
		...
		if (!SCManagerHandle) {
			wsprintf(errormsg,
				TEXT("Couldn't open the Service Manager!\nErrorcode: %d"),
				GetLastError());
